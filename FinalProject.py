...
def main():
    ...
    #reset function
        ...
        left_frame.place(relx=0.005,rely=0.01,width=610,height=465)
        ...
        right_frame.place(relx=0.46,rely=0.01,width=723,height=465)
        ...
        search_cb=ttk.Combobox(ru_frame,font=('times new roman',12),state='readonly',width=17,textvariable=var_searchcb)
        search_cb['values']=('Select','StudentID no.','Roll no.','Mobile no.')
        search_cb.current(0)
        search_cb.grid(row=0,column=1,padx=10,pady=10)

        search_tb=Entry(ru_frame,font=('times new roman',12),width=17,textvariable=var_searchtb)
        search_tb.grid(row=0,column=2,padx=10,pady=10)

        search_but=Button( ru_frame,text='Search',font=('times new roman',12),fg='white',bg='#9f1c33',relief=RAISED,cursor='hand2',width=8,borderwidth=3,command=search)
        search_but.grid(row=0,column=3,padx=10,pady=10)

        showall_but=Button( ru_frame,text='Show All',font=('times new roman',12),fg='white',bg='#9f1c33',relief=RAISED,cursor='hand2',width=8,borderwidth=3,command=fetch_data)
        showall_but.grid(row=0,column=4,padx=5,pady=10)

        #table frame
        rl_frame=Frame(right_frame,borderwidth=2,relief=SUNKEN)
        rl_frame.place(relx=0.01,rely=0.2,width=700,height=330)
    ...
    def atten_func(root=root):
        ...
        if len(fln)>0:
            ...
            # exportcsv button
            exportcsv_but=Button(ll_frame,text='Export CSV',font=('times new roman',12),fg='white',bg='#9f1c33',relief=RAISED,cursor='hand2',width=20,borderwidth=5,command=export_csv)
            exportcsv_but.grid(row=0,column=2,padx=0,pady=0)
            # update button
            Update_but=Button(ll_frame,text='Update',font=('times new roman',12),fg='white',bg='#9f1c33',relief=RAISED,cursor='hand2',width=20,borderwidth=5,command=updatecsv)
            Update_but.grid(row=0,column=0,padx=0,pady=0)
            # Reset button
            Reset_but=Button(ll_frame,text='Reset',font=('times new roman',12),fg='white',bg='#9f1c33',relief=RAISED,cursor='hand2',width=20,borderwidth=5,command=reset_csv)
            Reset_but.grid(row=0,column=1,padx=0,pady=0)

            #right frame
            right_frame=LabelFrame(mainframe,text='Attendance Sheet',bd=3,fg='#9f1c33',font=('times new roman',15),relief=RAISED,borderwidth=3)
